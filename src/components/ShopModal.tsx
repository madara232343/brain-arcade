...
export const ShopModal: React.FC<ShopModalProps> = ({ userProgress, onClose, onPurchase }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-2 md:p-4">
      <div className="bg-gradient-to-br from-indigo-900/95 to-purple-900/95 backdrop-blur-lg rounded-2xl md:rounded-3xl max-w-6xl w-full max-h-[95vh] overflow-hidden border border-white/30 shadow-2xl animate-scale-in">
        ...
        <div className="p-3 md:p-6 overflow-y-auto max-h-[calc(95vh-80px)] md:max-h-[calc(95vh-120px)]">
          {/* Category Tabs */}
          <div className="flex space-x-1 mb-4 md:mb-6 bg-white/10 rounded-xl p-1 overflow-x-auto">
            {(['powerups', 'characters', 'themes', 'inventory'] as const).map((category) => (
              <button
                ...
                className={`flex-shrink-0 py-2 px-3 md:px-4 rounded-lg font-medium transition-all flex items-center space-x-1 md:space-x-2 text-sm md:text-base ${
              ...
              return (
                ...
                >
                  <div className="text-center">
                    ...
                    <h3 className="font-bold text-white text-sm md:text-lg mb-2">{item.name}</h3>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {isOwned ? (
                        <span className="bg-green-500 text-white px-3 md:px-4 py-2 rounded-lg font-bold flex-1 text-center text-sm md:text-base">
                          ✓ Owned
                        </span>
                      ) : activeCategory !== 'inventory' ? (
                        <button
                          ...
                          disabled={!canAfford}
                          className={`px-3 md:px-4 py-2 rounded-lg font-bold transition-all touch-target text-sm md:text-base ${
                            canAfford
          ...
          {filteredItems.length === 0 && activeCategory === 'inventory' && (
            <div className="text-center py-8 md:py-12">
              <Package className="h-12 w-12 md:h-16 md:w-16 text-white/30 mx-auto mb-3 md:mb-4" />
